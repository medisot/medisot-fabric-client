...
const Channel = class {
	...
	async queryByChaincodeBySignedProposal(targets, proposal, timeout) {
		...
		const proposalResults = await sendTransactionProposalBySignedProposal(targets, proposal, timeout);
		const responses = proposalResults[0];
		logger.debug('queryByChaincodeBySignedProposal - results received');
		...
		responses.forEach((response) => {
			if (response instanceof Error) {
				...
				logger.error('queryByChaincodeBySignedProposal - unknown or missing results in query ::' + results);
				results.push(new Error(response));
			}
		});
